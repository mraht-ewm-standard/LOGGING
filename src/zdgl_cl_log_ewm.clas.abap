...
CLASS zdgl_cl_log_ewm DEFINITION
  PUBLIC
  INHERITING FROM zdgl_cl_log_sap FINAL
  CREATE PUBLIC.
  ...
  PROTECTED SECTION.
    CLASS-DATA mo_instance   TYPE REF TO zdgl_cl_log_ewm.
...
CLASS zdgl_cl_log_ewm IMPLEMENTATION.
  ...
  METHOD handle_error.
    ...
    MESSAGE e001(zdgl_log) INTO DATA(lv_msgtx) ##NEEDED.
    log_message( ).

    " Close existing log and create a new one for error handling
    save( ).

    DATA(lo_log_sap) = NEW zdgl_cl_log_ewm(
      iv_lgnum     = mv_lgnum
      iv_object    = zdgl_cl_log=>mc_default-log_object
