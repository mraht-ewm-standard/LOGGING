...
CLASS zdgl_cl_log_sap DEFINITION
  PUBLIC
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    CONSTANTS mc_class_name TYPE classname VALUE 'ZDGL_CL_LOG_SAP'.
    ...
    CONSTANTS: BEGIN OF mc_msgde_callback,
                 baluef      TYPE baluef VALUE 'ZDGL_FM_LOG_CALLBACK',
                 baluep      TYPE baluep VALUE 'ZDGL_R_BS_LOG_CALLBACK',
    ...
    METHODS constructor
      IMPORTING iv_object        TYPE balobj_d  DEFAULT zdgl_cl_log=>mc_default-log_object
                iv_subobject     TYPE balsubobj DEFAULT zdgl_cl_log=>mc_default-log_subobject
                iv_extnumber     TYPE balnrext  OPTIONAL
                it_extnumber     TYPE stringtab OPTIONAL
                iv_callstack_lvl TYPE numc1     DEFAULT zdgl_cl_log=>mc_callstack_lvl-info.
  ...
  PROTECTED SECTION.
    ...
    DATA mv_msg_param_id      TYPE zdgl_cl_log=>v_message_param_id.
    DATA ms_msg_details       TYPE zdgl_s_msg_details.
    DATA mt_msg_details       TYPE zdgl_tt_msg_details.
    ...
    CLASS-METHODS error_handling
      IMPORTING iv_process        TYPE char4
                ...
                is_log_msg        TYPE zdgl_s_log_msg
...
CLASS zdgl_cl_log_sap IMPLEMENTATION.
  ...
  METHOD add_message_callstack.
    ...
    CASE mv_msg_type.
      WHEN zdgl_cl_log=>mc_log_type-info.
        IF mv_callstack_lvl LT zdgl_cl_log=>mc_callstack_lvl-info.
          RETURN.
        ENDIF.

      WHEN zdgl_cl_log=>mc_log_type-success.
        IF mv_callstack_lvl LT zdgl_cl_log=>mc_callstack_lvl-success.
          RETURN.
        ENDIF.

      WHEN zdgl_cl_log=>mc_log_type-warning.
        IF mv_callstack_lvl LT zdgl_cl_log=>mc_callstack_lvl-warning.
          RETURN.
        ENDIF.

      WHEN zdgl_cl_log=>mc_log_type-error.
        IF mv_callstack_lvl LT zdgl_cl_log=>mc_callstack_lvl-error.
          RETURN.
        ENDIF.

    ENDCASE.

    zdgl_cl_session=>get_callstack( IMPORTING et_callstack = DATA(lt_callstack) ).
  ...
  METHOD add_message_context.
    ...
    zdgl_cl_session=>get_context( IMPORTING ev_program   = DATA(lv_program)
                                            ev_blockname = DATA(lv_include)
                                            ev_line      = DATA(lv_line) ).

    ms_msg_context = VALUE #( value   = VALUE zdgl_s_log_context( program = lv_program
                                                                  include = lv_include
                                                                  line    = lv_line )
                              tabname = zdgl_cl_log=>mc_log_context_struct ).
  ...
  METHOD add_message_detail.
    ...
    APPEND VALUE #( parname  = zdgl_cl_log=>mc_msg_ident
  ...
  METHOD add_msg_by_message_object.
    ...
    CASE sy-subrc.
      ...
      WHEN OTHERS.
        handle_error( iv_process = zdgl_cl_log=>mc_log_process-create
                      iv_subrc   = sy-subrc ).
  ...
  METHOD create_message.
    ...
    CASE mv_msg_content_type.
      WHEN zdgl_cl_log=>mc_msg_content_type-obj.
        add_msg_by_message_object( ).

      WHEN zdgl_cl_log=>mc_msg_content_type-txt.
  ...
  METHOD det_caller.
    ...
    zdgl_cl_session=>get_callstack( IMPORTING ev_function = DATA(lv_function)
  ...
  METHOD log_error.
    ...
    create_message( iv_msgty = zdgl_cl_log=>mc_log_type-error
                    iv_msgtx = iv_msgtx
                    iv_msgno = iv_msgno
                    iv_msgv1 = iv_msgv1
                    iv_msgv2 = iv_msgv2
  ...
  METHOD handle_error.
    ...
    " Backup input data
    DATA(ls_log_msg) = VALUE zdgl_s_log_msg( hdr = VALUE #( object     = ms_log_header-object
    ...
    MESSAGE e001(zdgl_log) INTO DATA(lv_msg) ##NEEDED.
    log_message( ).

    " Close existing log and create a new one for error handling
    save( ).

    DATA(lo_log_sap) = NEW zdgl_cl_log_sap( iv_object    = zdgl_cl_log=>mc_default-log_object
                                            iv_subobject = zdgl_cl_log=>mc_default-log_subobject
  ...
  METHOD error_handling.
    ...
    MESSAGE e017(zdgl_log) WITH iv_process INTO DATA(lv_msg) ##NEEDED.
    APPEND zdgl_cl_log=>to_bapiret( ) TO rt_bapiret.
    ...
    APPEND zdgl_cl_log=>to_bapiret( iv_msgty = zdgl_cl_log=>mc_log_type-error
                                    iv_msgtx = lv_msg_txt_gen ) TO rt_bapiret.

    CASE iv_process.
      WHEN zdgl_cl_log=>mc_log_process-init.
        CASE iv_subrc.
          WHEN 1.
            MESSAGE e006(zdgl_log) WITH iv_subrc INTO lv_msg.

          WHEN OTHERS.
            MESSAGE e007(zdgl_log) WITH iv_subrc INTO lv_msg.

        ENDCASE.

      WHEN zdgl_cl_log=>mc_log_process-save.
        CASE iv_subrc.
          WHEN 1.
            MESSAGE e008(zdgl_log) WITH iv_subrc INTO lv_msg.

          WHEN 2.
            MESSAGE e009(zdgl_log) WITH iv_subrc INTO lv_msg.

          WHEN 3.
            MESSAGE e010(zdgl_log) WITH iv_subrc INTO lv_msg.

          WHEN OTHERS.
            MESSAGE e011(zdgl_log) WITH iv_subrc INTO lv_msg.
      ...
      WHEN OTHERS.
        CASE sy-subrc.
          ...
          WHEN 2.
            MESSAGE e012(zdgl_log) WITH iv_subrc INTO lv_msg.

          WHEN 3.
            MESSAGE e013(zdgl_log) WITH iv_subrc INTO lv_msg.

          WHEN OTHERS.
            MESSAGE e014(zdgl_log) WITH iv_subrc INTO lv_msg.
    ...
    CASE iv_process.
      ...
      WHEN zdgl_cl_log=>mc_log_process-create.
        IF is_log_msg-msg-msgtx CN ' _0'.

          MESSAGE e015(zdgl_log) WITH is_log_msg-msg-msgty INTO lv_msg.
          APPEND zdgl_cl_log=>to_bapiret( ) TO rt_bapiret.

        ELSEIF is_log_msg-msg-msgno CN ' _0'.

          MESSAGE e003(zdgl_log) WITH is_log_msg-msg-msgno
                                      is_log_msg-msg-msgid INTO lv_msg.
          APPEND zdgl_cl_log=>to_bapiret( ) TO rt_bapiret.

          MESSAGE e004(zdgl_log) WITH is_log_msg-msg-msgv1
        ...
        ELSE.
          ...
          MESSAGE e002(zdgl_log) INTO lv_msg.
          APPEND zdgl_cl_log=>to_bapiret( ) TO rt_bapiret.

        ENDIF.

      WHEN zdgl_cl_log=>mc_log_process-exception.
        DATA(lo_exc_descr) = NEW cl_instance_description( io_exception ).

        MESSAGE e005(zdgl_log) WITH lo_exc_descr->class_name INTO lv_msg.
        APPEND zdgl_cl_log=>to_bapiret( ) TO rt_bapiret.
  ...
  METHOD info.
    ...
    create_message( iv_msgty = zdgl_cl_log=>mc_log_type-info
                    iv_msgtx = iv_msgtx
                    iv_msgno = iv_msgno
                    iv_msgv1 = iv_msgv1
                    iv_msgv2 = iv_msgv2
  ...
  METHOD log_caller.
    ...
    create_message( iv_msgty        = zdgl_cl_log=>mc_log_type-success
  ...
  METHOD log_duration.
    ...
    TRY.
        ...
        MESSAGE s018(zdgl_log) WITH lv_duration INTO DATA(lv_msgtx).
        create_message( iv_msgty        = zdgl_cl_log=>mc_log_type-success
  ...
  METHOD log_exception.
    ...
    CASE TYPE OF io_exception.
      ...
      WHEN OTHERS.
        INSERT zdgl_cl_log=>to_bapiret( iv_msgtx = CONV #( io_exception->get_text( ) ) ) INTO TABLE lt_messages.
  ...
  METHOD save_log.
    ...
    CASE sy-subrc.
      ...
      WHEN OTHERS.
        ...
        delete_log( ).
        handle_error( iv_process = zdgl_cl_log=>mc_log_process-save
                      iv_subrc   = lv_subrc ).
  ...
  METHOD set_content.
    ...
    IF iv_msgtx IS NOT INITIAL.
      ...
      mv_msg_content_type = zdgl_cl_log=>mc_msg_content_type-txt.
    ...
    ELSEIF iv_msgno CN ' _'.
      ...
      mv_msg_content_type = zdgl_cl_log=>mc_msg_content_type-obj.
  ...
  METHOD set_priority.
    ...
    CASE mv_msg_type.
      WHEN zdgl_cl_log=>mc_log_type-info.
        mv_msg_priority = zdgl_cl_log=>mc_log_type-info_prio.    " Additional information

      WHEN zdgl_cl_log=>mc_log_type-success.
        mv_msg_priority = zdgl_cl_log=>mc_log_type-success_prio. " Medium important

      WHEN zdgl_cl_log=>mc_log_type-warning.
        mv_msg_priority = zdgl_cl_log=>mc_log_type-warning_prio. " Important

      WHEN zdgl_cl_log=>mc_log_type-error.
        mv_msg_priority = zdgl_cl_log=>mc_log_type-error_prio.   " Very important
  ...
  METHOD log_warning.
    ...
    create_message( iv_msgty = zdgl_cl_log=>mc_log_type-warning
                    iv_msgtx = iv_msgtx
                    iv_msgno = iv_msgno
                    iv_msgv1 = iv_msgv1
                    iv_msgv2 = iv_msgv2
  ...
  METHOD create_log.
    ...
    IF     sy-subrc     NE 0
      ...
      handle_error( iv_process = zdgl_cl_log=>mc_log_process-init
                    iv_subrc   = sy-subrc ).
    ENDIF.
  ...
  METHOD on_log_callback.
    ...
    DATA lv_msg_param_id   TYPE zdgl_cl_log=>v_message_param_id.
    ...
    DATA(lt_msg_details) = VALUE zdgl_tt_msg_details( ).
    IMPORT msg_details TO lt_msg_details FROM DATABASE bal_indx(al) ID lv_log_number.
    IF sy-subrc EQ 4.
      MESSAGE s019(zdgl_log) DISPLAY LIKE 'E'.
    ENDIF.

    CHECK lt_msg_details IS NOT INITIAL.

    lv_msg_param_id = VALUE #( it_params[ param = zdgl_cl_log=>mc_msg_ident ]-value OPTIONAL ).
    ...
    IF zdgl_cl_log=>mo_gui_alv_grid IS NOT INITIAL.
      zdgl_cl_log=>mo_gui_alv_grid->free( ).
      CLEAR zdgl_cl_log=>mo_gui_alv_grid.
    ENDIF.

    "    Show container if not visible
    " OR Hide container if detail to same message was again being selected
    IF     zdgl_cl_log=>mo_gui_docking_container IS BOUND
       AND zdgl_cl_log=>mv_sel_msg_param_id      EQ lv_msg_param_id.

      zdgl_cl_log=>mo_gui_docking_container->free( ).
      CLEAR: zdgl_cl_log=>mo_gui_docking_container,
             zdgl_cl_log=>mv_sel_msg_param_id.

    ELSEIF zdgl_cl_log=>mo_gui_docking_container IS NOT BOUND.

      zdgl_cl_log=>mo_gui_docking_container = NEW #( side      = cl_gui_docking_container=>dock_at_bottom
                                                     extension = '120' ).
      zdgl_cl_log=>mo_gui_docking_container->set_visible( abap_true ).

    ENDIF.

    CHECK zdgl_cl_log=>mo_gui_docking_container IS BOUND.

    zdgl_cl_log=>mo_gui_alv_grid = NEW #( i_parent = zdgl_cl_log=>mo_gui_docking_container ).
    ...
    zdgl_cl_log=>mo_gui_alv_grid->set_table_for_first_display( EXPORTING i_structure_name     = ls_structure_name
                                                                         is_layout            = ls_layout
                                                                         it_toolbar_excluding = lt_alv_fcodes_excl
                                                               CHANGING  it_outtab            = <lt_outtab> ).

    zdgl_cl_log=>mv_sel_msg_param_id = lv_msg_param_id.
