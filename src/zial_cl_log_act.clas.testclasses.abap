...
CLASS ltc_log_act DEFINITION FINAL
  CREATE PUBLIC
  ...
  PRIVATE SECTION.
    ...
    TYPES: BEGIN OF s_tdc_data,
             t_log_act TYPE zial_tt_log_act,
           END OF s_tdc_data.
...
CLASS ltc_log_act IMPLEMENTATION.

  METHOD class_setup.

    mo_aunit = zial_cl_aunit=>on_class_setup( iv_tdc_cnt  = mc_tdc_cnt
                                              ir_tdc_data = REF #( ms_tdc_data )
                                              it_sql_data = VALUE #( ( tbl_name = 'ZIAL_T_LOG_ACT'
                                                                       tbl_data = REF #( ms_tdc_data-t_log_act ) ) ) ).
  ...
  METHOD t0001.
    ...
    LOOP AT ms_tdc_data-t_log_act ASSIGNING FIELD-SYMBOL(<ls_log_act_exp>).
      ...
      DATA(ls_log_act_act) = zial_cl_log_act=>get( iv_object    = <ls_log_act_exp>-object
                                                   iv_subobject = <ls_log_act_exp>-subobject
                                                   iv_uname     = <ls_log_act_exp>-uname ).

      cl_abap_unit_assert=>assert_equals( exp = <ls_log_act_exp>
                                          act = ls_log_act_act ).
  ...
  METHOD t0002.
    ...
    cl_abap_unit_assert=>assert_initial( act = ls_log_act_act ).
