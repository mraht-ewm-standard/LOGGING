...
CLASS ziot_cl_log_ewm DEFINITION
  ...
  PUBLIC SECTION.
    ...
    CONSTANTS: callstack_level_none    TYPE numc1 VALUE 0,
               callstack_level_error   TYPE numc1 VALUE 1,
               callstack_level_warning TYPE numc1 VALUE 2,
               callstack_level_success TYPE numc1 VALUE 3,
               callstack_level_info    TYPE numc1 VALUE 4.

    CONSTANTS: log_type_error   TYPE symsgty VALUE 'E',
               log_type_warning TYPE symsgty VALUE 'W',
               log_type_success TYPE symsgty VALUE 'S',
               log_type_info    TYPE symsgty VALUE 'I'.
    ...
    "! @parameter io_sap_log | SAP log instance
    ...
    CLASS-METHODS init
      IMPORTING
        ...
        !iv_object          TYPE balobj_d DEFAULT '/SCWM/WME' " mc_dflt_log_object
        !iv_subobject       TYPE balsubobj
        !iv_extnumber       TYPE balnrext OPTIONAL
        !it_extnumber_list  TYPE stringtab OPTIONAL
        !io_sap_log         TYPE REF TO /scwm/cl_log OPTIONAL
        !iv_callstack_level TYPE numc1 DEFAULT callstack_level_info
      RETURNING
        VALUE(ro_instance)  TYPE REF TO ziot_cl_log.
    ...
    CLASS-METHODS display_appl_msg
      IMPORTING
        msg   TYPE string
        msgty TYPE msgty DEFAULT ziot_cl_log=>log_type_success
    ...
    "! Append SAP log object which is then automatically
    "! being updated on base of the related log instance
    "!
    "! @parameter io_log | SAP log object
    METHODS set_sap_log
      IMPORTING
        !io_log TYPE REF TO /scwm/cl_log OPTIONAL.
    METHODS set_callstack_level
      IMPORTING
        iv_callstack_level TYPE numc1.
  ...
  PROTECTED SECTION.
    ...
    DATA callstack_level TYPE numc1.
...
CLASS ZIOT_CL_LOG_EWM IMPLEMENTATION.
  ...
  METHOD add_message_callstack.
    ...
    CASE message_type.
      WHEN log_type_info.
        CHECK callstack_level >= callstack_level_info.

      WHEN log_type_success.
        CHECK callstack_level >= callstack_level_success.

      WHEN log_type_warning.
        CHECK callstack_level >= callstack_level_warning.

      WHEN log_type_error.
        CHECK callstack_level >= callstack_level_error.

    ENDCASE.

    ziot_cl_bs_session=>get_callstack(
      IMPORTING
...
*** Prozessbedingte Log-Daten loggen
  ...
  METHOD get_instance.
    ...
    IF log_stack IS INITIAL.
      ...
      instance = ziot_cl_log=>init( iv_lgnum     = ziot_if_const=>lgnum
                                    iv_object    = mc_dflt_log_object
                                    iv_subobject = mc_log_subobject_log
  ...
  METHOD init.
    ...
    instance->set_callstack_level( iv_callstack_level ).
  ...
  METHOD set_callstack_level.

    me->callstack_level = iv_callstack_level.
